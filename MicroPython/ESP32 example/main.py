...
SERVER_SSID = 'RoboKernel-TANK'  # max 32 characters
SERVER_IP = '10.0.0.1'
SERVER_SUBNET = '255.255.255.0'

async def receiver():
    sreader = asyncio.StreamReader(myUsart)
    while True:
...
#         loop.create_task(self.run_dns_server_print_uart())
        
#         loop.create_task(self.check_and_send())
    ...
    async def handle_http_connection(self, reader, writer):
        ...
        mv_start = request_line.find('?action=start')
        mv_back = request_line.find('?action=back')
        mv_right = request_line.find('?action=right')
        mv_left = request_line.find('?action=left')
        mv_stop = request_line.find('?action=stop')
        mv_mode = request_line.find('?action=mode')
